...
{uint8_t buffer[64]; 
  ...
  if (enableAccel) // If accelerometer logging is enabled
  {
    ...
    {  tmp = imu.calcAccel(imu.ax);
       memcpy(buffer+index, &tmp, len); index+=8; 
       tmp = imu.calcAccel(imu.ay);
       memcpy(buffer+index, &tmp , len); index+=8;  
       tmp = imu.calcAccel(imu.az);
       memcpy(buffer+index, &tmp , len); index+=8; 
    }
    else
    {  tmp = imu.ax; 
...
void initHardware(void)
{
  ...
  //Set up Firefly interrupt pin input (active-low)
  pinMode(FF_INT_PIN, INPUT_PULLUP); 
